...
"""
Creates a spark session capable of interacting with s3 files.
"""
...
"""
Reads S3 files from "input_data" and converts it to
a "song_table" and "artists_table" parquet files.
"""
def process_song_data(spark, input_data, output_data):
    ...
    songs_table.write.mode("overwrite").partitionBy("year", "artist_id").parquet("{}songs/songs_table.parquet".format(output_data))

    # extract columns to create artists table
    artists_table = df.select('artist_id', 'artist_name', 'artist_location', 'artist_latitude', 'artist_longitude').dropDuplicates()
    
    # write artists table to parquet files
    artists_table.write.mode("overwrite").parquet("{}artists/artists_table.parquet".format(output_data))

"""
Reads S3 files from "input_data" and converts it to
a "users_table", "time_table" and "songplays_table" parquet files.
"""
def process_log_data(spark, input_data, output_data):
    ...
    users_table.write.mode("overwrite").parquet('{}users/users_table.parquet'.format(output_data))
    ...
    time_table.write.mode("overwrite").partitionBy('year', 'month').parquet('{}time/time_table.parquet'.format(output_data))
    ...
    songplays_table.write.mode("overwrite").partitionBy("year", "month").parquet("{}songplays/songplays_table.parquet".format(output_data))

"""
Creates a Spark Session and initializes functions to create 
